<%-- 서블릿 주석 : JSP에서 page는 현재 보여지는 페이지에 대한 설정 --%>
...
[page]
a. lannguage : 사용할 언어 지정. java만 지정
b. import : 패키지를 import할 때 사용
c. errorPage : 설정된 페이지에서 에러가 발생했을 때에 미리 만들어 둔 에러 페이지를 호출
d. contentType : text/html 이렇게 사용되면 HTML 문서
e. pagerEncoding : 출력 문자 인코딩.
[include] : jsp에 다른 특정 영역에 속한 문서를 로드할 때 사용
ex) <%@ include file="header.jsp"%>
[taglib] : jsp 페이지에 사용할 태그 라이브러리를 지정함. - jstl 태그 라이브러리가 존재.
ex) <%@ taglib uri="http://java.sun.com/jsp/jstl/core" perfix="c"%>
3. 선언자 (delaration) : <%! %> 변수나 메서드 선언시 사용
4. 표현식 (expression) : <%= %> 결과값을 출력할 때 사용
5. 주석 (comments) : <%-- --%> 주석

--%>
<%
    // request 객체에 setAttribute로 dan, results를 넘겨서 받았음.
