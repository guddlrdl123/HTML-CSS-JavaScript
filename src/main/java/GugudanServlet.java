...
public class GugudanServlet extends HttpServlet {
    ...
    // GET 방식으로 들어오는 경우 처리하는 메서드
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("GET 방식으로 접근할 때 동작하는 메서드");
        // 기능 구현..

        // getParameter() 메서드는 request에 있는 파라미터 값을 읽어들이는 메서드. 인자로 파라미터 이름을 지정
        // ~~~/gugudan.do?dan=7
        String danStr = req.getParameter("dan"); // 7이라는 문자열을 넘김

        // 3항 연산을 이용해서 dan 값이 없거나 비어있는 경우에는 2로 처리, 이외는 해당 단으로 처리
        ...
        // http.HttpServletRequest.setAttribute("파라미터 이름", 값)
        // setAttribyute는 Attribute 객체에 값을 설정해 주는 메서드. (강제 설정)
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        System.out.println("POST 방식으로 접근할 때 동작하는 메서드");
